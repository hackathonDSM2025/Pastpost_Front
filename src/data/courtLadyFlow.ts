export interface CourtLadyChoice {
  text: string;
  nextId: string;
  answer?: string;
}

export interface CourtLadyScene {
  id: string;
  text: string | string[];
  image?: string;
  choices?: CourtLadyChoice[];
}
...
export const courtLadyFlow: Record<string, CourtLadyScene> = {
  start: {
    ...
    choices: [
      { text: "일어난다", nextId: "work_select", answer: "부지런한 궁녀네요!" },
      { text: "잔다", nextId: "scolded", answer: "지각하면 혼나요!" },
    ],
  },
  work_select: {
    id: "work_select",
    ...
    choices: [
      ...
      { text: "빨래", nextId: "laundry", answer: "우물가에서 빨래를 합니다." },
    ],
  },
  scolded: {
    id: "scolded",
